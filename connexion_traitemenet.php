<?php 
    ...
    if(isset($_POST['email'])  && isset($_POST['password'])) // Si il existe les champs email, password et qu'il sont pas vide
    {
        // Patch XSS
        ...
        $check = $bdd->prepare('SELECT * FROM utilisateurs.ID_utilisateurs = roles.ID_utilisateurs WHERE adresse_mail_utilise = ?');
            ...
            if(filter_var($email, FILTER_VALIDATE_EMAIL))
            {
               ...
               /* $password = hash('sha512', $password);
                ...
                if(password_verify($password,$hash ))
                {
                    ...
                    $cookieFin = time()+60*60*24; //Mise en place du timer du cookie
